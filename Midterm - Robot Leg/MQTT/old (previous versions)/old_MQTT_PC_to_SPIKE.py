# !!! change angles to relative (not absolute) once double check in Slack and test preliminary code (need to calc relative in excel first, then import NEW csv.
...
def make_thetas_tuple_as_strings(thetas):
    ...
    for position in thetas:
        this_hip_theta = int(position[0]) # convert float thetas to ints, in case the motors can't take floats so don't get error!
        this_knee_theta = int(position[1])
...
def send_thetas_to_broker(topic, thetas_tuple_to_send):
    
    # initialize as default string before putting in angles later
    message = '(0,0)' 

    wait_btwn_thetas = 1 # seconds
    # --- PUBLISH ---
...
def main():
    ...
    send_thetas_to_broker(topic_publish_to, thetas_to_send)
