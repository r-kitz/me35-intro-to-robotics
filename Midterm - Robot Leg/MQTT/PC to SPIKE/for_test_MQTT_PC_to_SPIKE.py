...
def make_thetas_tuple_as_strings(thetas):
    ...
    for position in thetas:
        ...
        if first_time:
            this_hip_theta -= 90 # to put on downwards zero axis
        
        # for Chris' set-up had to flip knee to first and upper(hip) to second
        this_position_as_str = '(' + str(this_knee_theta) + ',' +  str(this_hip_theta) + ')'
...
def send_thetas_to_broker(client, topic, thetas_tuple_to_send):
    ...
    wait_btwn_thetas = .5 # seconds, made longer for testing than during project process to ensure I could check if the motors were moving to the proper angles
    # --- PUBLISH ---
